...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        if len(self.cells) == self.count and self != 0:
            return self.cells
        return set()
...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i, j) in self.mines:
                    countMines += 1
                if 0 <= 1 < self.height and 0 <= 1 < self.width and (i, j) not in self.moves_made and (i, j) not in self.mines:
        ...
        for sentence in self.knowledge:
            if newSentence.cells.issubset(sentence.cells) and self.count > 0 and newSentence.count > 0 and newSentence != sentence:
                newSubset = sentence.cells.diffrence(newSentence.cells)
    ...
    def make_random_move(self):
        ...
        """
        #Bu kısımda yapılabilecek olası hamleler hesaplandı
        #Eğer istenen işlem yapılmadıysa, rastgele bir hamle seç
